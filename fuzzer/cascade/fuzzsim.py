...
def basic_runsim_verilator(design_name, simlen, elfpath, num_int_regs: int = MAX_NUM_PICKABLE_REGS-1, num_float_regs: int = MAX_NUM_PICKABLE_FLOATING_REGS, coveragepath = None, get_rfuzz_coverage_mask = False, simulator_executable: str = None):
    if DO_ASSERT:
    ...
    if simulator_executable:
        sim_executable_path=os.path.abspath(simulator_executable)
    else:
...
# @param get_rfuzz_coverage_mask if True, then return a pair (is_stop_successful: bool, rfuzz_coverage_mask: int)
# Return a pair (is_stop_successful: bool, reg_vals: int list of length <= MAX_NUM_PICKABLE_REGS-1 or None if is_stop_successful is False)
def runsim_verilator(design_name, simlen, elfpath, num_int_regs: int = MAX_NUM_PICKABLE_REGS-1, num_float_regs: int = MAX_NUM_PICKABLE_FLOATING_REGS, coveragepath = None, get_rfuzz_coverage_mask = False, simulator_executable: str = None):
    if DO_ASSERT:
        assert coveragepath is None or not get_rfuzz_coverage_mask
...
def basic_runtest_simulator(fuzzerstate, elfpath: str, expected_regvals: tuple, override_num_instrs: int = None, simulator=SimulatorEnum.VERILATOR, simulator_executable: str = None):
    expected_intregvals, expected_floatregvals = None, None
    ...
    if simulator == SimulatorEnum.VERILATOR:
        is_stop_successful, received_regvals = basic_runsim_verilator(fuzzerstate.design_name, num_instrs*MAX_CYCLES_PER_INSTR + SETUP_CYCLES, elfpath, fuzzerstate.num_pickable_regs-1, fuzzerstate.num_pickable_floating_regs, simulator_executable=simulator_executable)
...
# @param override_num_instrs if not None, then use this value instead of the number of instructions in fuzzerstate.instr_objs_seq. Used when pruning to shorten a bit the timeout.
# @return (is_success: bool, msg: str)
def runtest_simulator(fuzzerstate, elfpath: str, expected_regvals: tuple, override_num_instrs: int = None, simulator=SimulatorEnum.VERILATOR, simulator_executable: str = None):
    ...
    num_instrs = override_num_instrs if override_num_instrs is not None else len(list(itertools.chain.from_iterable(fuzzerstate.instr_objs_seq)))
    if simulator == SimulatorEnum.VERILATOR:
        is_stop_successful, received_regvals = runsim_verilator(fuzzerstate.design_name, num_instrs*MAX_CYCLES_PER_INSTR + SETUP_CYCLES, elfpath, fuzzerstate.num_pickable_regs-1, fuzzerstate.num_pickable_floating_regs, simulator_executable=simulator_executable)
    elif simulator == SimulatorEnum.MODELSIM:
        is_stop_successful, received_regvals = runsim_modelsim(fuzzerstate.design_name, num_instrs*MAX_CYCLES_PER_INSTR + SETUP_CYCLES, elfpath, fuzzerstate.num_pickable_regs-1, fuzzerstate.num_pickable_floating_regs)
    else:
        raise NotImplementedError(f"Unknown simulator {simulator}")
...
def runtest_verilator_forprofiling(fuzzerstate, elfpath: str, expected_fuzzerstate_len_fordebug: int, simulator_executable: str = None):
    if DO_ASSERT:
    ...
    is_stop_successful, received_regvals = runsim_verilator(fuzzerstate.design_name, len(fuzzerstate.instr_objs_seq[0])*MAX_CYCLES_PER_INSTR + SETUP_CYCLES, elfpath, 1, 0, simulator_executable=simulator_executable)
    # Check successful stop
    if not is_stop_successful:
...
def runtest_verilator_forrfuzz(fuzzerstate, elfpath: str, simulator_executable: str = None):
    num_instrs = len(list(itertools.chain.from_iterable(fuzzerstate.instr_objs_seq)))
    is_stop_successful, rfuzz_coverage_mask = runsim_verilator(fuzzerstate.design_name, num_instrs*MAX_CYCLES_PER_INSTR + SETUP_CYCLES, elfpath, 1, 0, get_rfuzz_coverage_mask=True, simulator_executable=simulator_executable)
    # Check successful stop
    if not is_stop_successful:
