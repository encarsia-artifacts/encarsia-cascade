...
from cascade.basicblock import gen_basicblocks, basic_gen_basicblocks
from cascade.fuzzsim import SimulatorEnum, runtest_simulator, basic_runtest_simulator
...
def basic_gen_fuzzerstate_elf_expectedvals(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, check_pc_spike_again: bool, max_num_instructions: int = None, no_dependency_bias: bool = False):
    ...
    print(f"riscv64-unknown-elf-objdump -D {rtl_elfpath}")
...
def basic_run_rtl(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, check_pc_spike_again: bool, nmax_instructions: int = None, nodependencybias: bool = False, simulator=SimulatorEnum.VERILATOR, simulator_executable: str = None):
    ...
    start = time.time()
    is_success, rtl_msg = runtest_simulator(fuzzerstate, rtl_elfpath, finalregvals_spikeresol, simulator=simulator, simulator_executable=simulator_executable)
...
def run_rtl(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, check_pc_spike_again: bool, nmax_instructions: int = None, nodependencybias: bool = False, simulator=SimulatorEnum.VERILATOR, simulator_executable: str = None):
    fuzzerstate, rtl_elfpath, finalregvals_spikeresol, time_seconds_spent_in_gen_bbs, time_seconds_spent_in_spike_resol, time_seconds_spent_in_gen_elf = gen_fuzzerstate_elf_expectedvals(memsize, design_name, randseed, nmax_bbs, authorize_privileges, check_pc_spike_again, nmax_instructions, nodependencybias)
...
def basic_test(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, loggers: list = None, check_pc_spike_again: bool = False, start_time: float = None, simulator_executable: str = None):
    is_success = basic_run_rtl(memsize, design_name, randseed, nmax_bbs, authorize_privileges, check_pc_spike_again, simulator_executable=simulator_executable)
    return is_success


# This function runs a single test run from a test descriptor (memsize, design_name, randseed, nmax_bbs) and returns the gathered times (used for the performance evaluation plot).
# Loggers are not yet very tested facilities.
def fuzz_single_from_descriptor(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, loggers: list = None, check_pc_spike_again: bool = False, nothing: float = None, simulator_executable: str = None, start_time: float = None):
    try:
        print(f"Running test{' at ' + str(datetime.datetime.now()-start_time) if start_time != None else ''} for params memsize: `{memsize}`, design_name: `{design_name}`, check_pc_spike_again: `{check_pc_spike_again}`, randseed: `{randseed}`, nmax_bbs: `{nmax_bbs}`, authorize_privileges: `{authorize_privileges}` -- ({memsize}, {design_name}, {randseed}, {nmax_bbs}, {authorize_privileges})", flush=True)
        gathered_times = run_rtl(memsize, design_name, randseed, nmax_bbs, authorize_privileges, check_pc_spike_again, simulator_executable=simulator_executable)
        ...
        if loggers is not None:
            ...
            print(f"Failed test_run_rtl_single{' at ' + str(datetime.datetime.now()-start_time) if start_time != None else ''} for params memsize: `{memsize}`, design_name: `{design_name}`, check_pc_spike_again: `{check_pc_spike_again}`, randseed: `{randseed}`, nmax_bbs: `{nmax_bbs}`, authorize_privileges: `{authorize_privileges}` -- ({memsize}, {design_name}, {randseed}, {nmax_bbs}, {authorize_privileges})\n{e}", flush=True)
        return 0, 0, 0, 0
