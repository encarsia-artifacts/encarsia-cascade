...
def is_mismatch(fuzzerstate, max_bb_id_to_consider: int, failing_instr_id: int = None, index_first_bb_to_consider: int = 1, index_first_instr_to_consider: int = 0, quiet: bool = False, simulator_executable: str = None):
    # try:
    ...
    del fuzzerstate
    is_success, rtl_msg = runtest_simulator(test_fuzzerstate, rtl_elfpath, expected_regvals_pair, numinstrs, REDUCTION_SIMULATOR, simulator_executable=simulator_executable)
...
def _try_flatten_cf(fuzzerstate, simulator_executable: str = None):
    ...
    is_flattening_success = is_mismatch(flat_fuzzerstate, 1, simulator_executable=simulator_executable)
...
def _find_failing_bb(fuzzerstate, hint_left_bound_bb: int = None, hint_right_bound_bb: int = None, simulator_executable: str = None):
    # Take the hints
    if hint_left_bound_bb is not None:
        if DO_ASSERT:
            ...
            assert not is_mismatch(fuzzerstate, hint_left_bound_bb, simulator_executable=simulator_executable), f"Wrong left bound hint `{hint_left_bound_bb}`."
    ...
    if hint_right_bound_bb is not None:
        if DO_ASSERT:
            ...
            assert is_mismatch(fuzzerstate, hint_right_bound_bb, simulator_executable=simulator_executable), f"Wrong right bound hint `{hint_right_bound_bb}`."
    ...
    while right_bound - left_bound > 1:
        ...
        if is_mismatch(fuzzerstate, candidate_bound, simulator_executable=simulator_executable):
...
def _find_pillar_bb(fuzzerstate, failing_bb_id: int, failing_instr_id: int, fault_from_prev_bb: bool, hint_left_bound_pillar_bb: int = None, hint_right_bound_pillar_bb: int = None, simulator_executable: str = None):
    if DO_ASSERT:
    ...
    if hint_left_bound_pillar_bb is not None:
        if DO_ASSERT:
            ...
            assert is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, hint_left_bound_pillar_bb, simulator_executable=simulator_executable), f"Wrong left bound hint `{hint_left_bound_pillar_bb}`."
    ...
    if hint_right_bound_pillar_bb is not None:
        if DO_ASSERT:
            ...
            assert not is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, hint_right_bound_pillar_bb, simulator_executable=simulator_executable), f"Wrong right bound hint `{hint_right_bound_pillar_bb}`."
    ...
    while right_bound - left_bound > 1:
        if DO_ASSERT:
            assert right_bound > left_bound
        candidate_bound = (right_bound + left_bound) // 2
        if is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, candidate_bound, simulator_executable=simulator_executable):
...
def _find_failing_instr_in_bb(fuzzerstate, failing_bb_id: int, hint_left_bound_instr: int = None, hint_right_bound_instr: int = None, simulator_executable: str = None):
    if DO_ASSERT:
    ...
    if hint_left_bound_instr is not None:
        if DO_ASSERT:
            ...
            assert not is_mismatch(fuzzerstate, failing_bb_id, hint_left_bound_instr, simulator_executable=simulator_executable), f"Wrong left bound hint `{hint_left_bound_instr}`."
    ...
    if hint_right_bound_instr is not None:
        if DO_ASSERT:
            ...
            assert is_mismatch(fuzzerstate, failing_bb_id, hint_right_bound_instr, simulator_executable=simulator_executable), f"Wrong right bound hint `{hint_right_bound_instr}`."
    ...
    if right_bound == 0 or is_mismatch(fuzzerstate, failing_bb_id, 0, simulator_executable=simulator_executable):
    ...
    while right_bound - left_bound > 1:
        if DO_ASSERT:
            assert right_bound > left_bound
        candidate_bound = (right_bound + left_bound) // 2
        if is_mismatch(fuzzerstate, failing_bb_id, candidate_bound, simulator_executable=simulator_executable):
...
def _find_pillar_instr(fuzzerstate, failing_bb_id: int, failing_instr_id: int, pillar_bb_id: int, fault_from_prev_bb: bool, hint_left_pillar_instr: int = None, hint_right_pillar_instr: int = None, simulator_executable: str = None):
    if DO_ASSERT:
    ...
    if hint_left_pillar_instr is not None:
        if DO_ASSERT:
            ...
            assert is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, hint_left_pillar_instr, simulator_executable=simulator_executable), f"Wrong left bound hint `{hint_left_pillar_instr}`."
    ...
    if hint_right_pillar_instr is not None:
        if DO_ASSERT:
            ...
            assert not is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, hint_right_pillar_instr, simulator_executable=simulator_executable), f"Wrong right bound hint `{hint_right_pillar_instr}`."
    ...
    while right_bound - left_bound > 1:
        if DO_ASSERT:
            assert right_bound > left_bound
        candidate_bound = (right_bound + left_bound) // 2
        if is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, candidate_bound, simulator_executable=simulator_executable):
...
def _turn_sandwich_instructions_into_nops(fuzzerstate, failing_bb_id: int, failing_instr_id: int, pillar_bb_id: int, pillar_instr: int, fault_from_prev_bb: bool, simulator_executable: str = None):
    if DO_ASSERT:
        assert pillar_bb_id <= failing_bb_id+1
    ...
    if DO_ASSERT:
        assert is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, pillar_instr, simulator_executable=simulator_executable)
        assert not is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, pillar_instr+1, simulator_executable=simulator_executable)
    ...
    if pillar_bb_id == failing_bb_id:
        ...
        for instr_id in range(pillar_instr, failing_instr_id+1):
            ...
            try:
                if is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, pillar_instr, simulator_executable=simulator_executable):
...
def reduce_program(memsize: int, design_name: str, randseed: int, nmax_bbs: int, authorize_privileges: bool, find_pillars: bool, quiet: bool = False, target_dir: str = None, hint_left_bound_bb: int = None, hint_right_bound_bb: int = None, hint_left_bound_instr: int = None, hint_right_bound_instr: int = None, hint_left_bound_pillar_bb: int = None, hint_right_bound_pillar_bb: int = None, hint_left_bound_pillar_instr: int = None, hint_right_bound_pillar_instr: int = None, check_pc_spike_again: bool = False, simulator_executable: str = None):
    ...
    if len(fuzzerstate.instr_objs_seq) == 1:
        ...
        print('Is mismatch', is_mismatch(fuzzerstate, 1, len(fuzzerstate.instr_objs_seq[0])-1, simulator_executable=simulator_executable))
    ...
    failing_bb_id = _find_failing_bb(fuzzerstate, hint_left_bound_bb, hint_right_bound_bb, simulator_executable=simulator_executable)

    # If fail even just with the initial block
    if failing_bb_id == 0:
        if is_mismatch(fuzzerstate, 1, len(fuzzerstate.instr_objs_seq[0])-1, simulator_executable=simulator_executable):
            if not quiet:
    ...
    # If no fail at all
    if failing_bb_id == len(fuzzerstate.instr_objs_seq) and not is_mismatch(fuzzerstate, failing_bb_id-1, simulator_executable=simulator_executable):
        if not quiet:
    ...
    failing_instr_id = _find_failing_instr_in_bb(fuzzerstate, failing_bb_id, hint_left_bound_instr, hint_right_bound_instr, simulator_executable=simulator_executable)
    ...
    if failing_instr_id is None:
        ...
        failing_instr_id = -1
        if not is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, simulator_executable=simulator_executable):
            print('Detected interaction between CF instruction and the next block instruction')
            failing_instr_id = 0
            assert is_mismatch(fuzzerstate, failing_bb_id, failing_instr_id, simulator_executable=simulator_executable)
    ...
    if find_pillars:
        pillar_bb_id = _find_pillar_bb(fuzzerstate, failing_bb_id, failing_instr_id, fault_from_prev_bb, hint_left_bound_pillar_bb, hint_right_bound_pillar_bb, simulator_executable=simulator_executable)
    else:
        pillar_bb_id = 1
    ...
    if find_pillars:
        pillar_instr = _find_pillar_instr(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, fault_from_prev_bb, hint_left_bound_pillar_instr, hint_right_bound_pillar_instr, simulator_executable=simulator_executable)
    else:
        pillar_instr = 0
    ...
    if find_pillars and NOPIZE_SANDWICH_INSTRUCTIONS:
        ...
        fuzzerstate = _turn_sandwich_instructions_into_nops(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, pillar_instr, fault_from_prev_bb, simulator_executable=simulator_executable)
    ...
    if FLATTEN_SANDWICH_INSTRUCTIONS and not pillar_bb_id == failing_bb_id:
        ...
        try:
            fuzzerstate, is_success_flattening = _try_flatten_cf(fuzzerstate, simulator_executable=simulator_executable)
        except Exception as e:
        ...
        if is_success_flattening:
            ...
            failing_instr_id = _find_failing_instr_in_bb(fuzzerstate, failing_bb_id, simulator_executable=simulator_executable)
            pillar_instr = _find_pillar_instr(fuzzerstate, failing_bb_id, failing_instr_id, pillar_bb_id, fault_from_prev_bb, simulator_executable=simulator_executable)
    ...
    is_success_larger, rtl_msg_larger = runtest_simulator(test_fuzzerstate_larger, rtl_elfpath_larger, expected_regvals_pairs_larger, numinstrs_smaller, simulator_executable=simulator_executable)
    if not quiet:
        print('Success larger:', 0, is_success_larger)
        print('larger msg:', rtl_msg_larger)
    is_success_smaller, rtl_msg_smaller = runtest_simulator(test_fuzzerstate_smaller, rtl_elfpath_smaller, expected_regvals_pairs_smaller, numinstrs_larger, simulator_executable=simulator_executable)
    if not quiet:
