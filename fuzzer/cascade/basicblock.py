...
from cascade.finalblock import get_finalblock_max_size,finalblock,basic_finalblock
...
def basic_gen_basicblocks(fuzzerstate):
    # Until the generation succeeds

    fuzzerstate.reset()

    fuzzerstate.init_new_bb()
    final_instrs = basic_finalblock(fuzzerstate, fuzzerstate.design_name)

    for instr in final_instrs:
        fuzzerstate.instr_objs_seq[-1].append(instr)
    # fuzzerstate.bb_start_addr_seq.append(fuzzerstate.curr_bb_start_addr)

    print(f"fuzzerstate.instr_objs_seq: {fuzzerstate.instr_objs_seq}")
    print(f"fuzzerstate.bb_start_addr_seq: {fuzzerstate.bb_start_addr_seq}")
