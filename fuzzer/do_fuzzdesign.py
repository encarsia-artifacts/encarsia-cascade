...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        raise Exception("Usage: python3 do_fuzzdesign.py <design_name> <num_cores> <seed_offset> <authorize_privileges> <tolerate_some_bug> <simulator_executable>")
    ...
    if len(sys.argv) > 6:
        simulator_executable = str(sys.argv[6])
    else:
        simulator_executable = None

    fuzzdesign(sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), authorize_privileges, simulator_executable)
