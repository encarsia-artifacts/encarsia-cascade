...
def __get_medeleg_mask(design_name: str, simulator_executable: str = None):
    ...
    return runtest_verilator_forprofiling(fuzzerstate, rtl_elfpath, 1, simulator_executable=simulator_executable)
...
def profile_get_medeleg_mask(design_name: str, simulator_executable: str = None):
    if design_name in ["picorv32", "ibex"]:
        return 0 # This design does not support medeleg
    global PROFILED_MEDELEG_MASK
    PROFILED_MEDELEG_MASK = __get_medeleg_mask(design_name, simulator_executable=simulator_executable)
